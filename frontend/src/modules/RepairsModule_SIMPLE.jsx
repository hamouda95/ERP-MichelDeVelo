/**
 * ============================================================================ 
 * REPAIRS MODULE - VERSION SIMPLIFIÉE SANS DRAG & DROP
 * ============================================================================ 
 * 
 * Cette version utilise des boutons pour changer le statut
 * au lieu du drag & drop pour éviter les problèmes de react-beautiful-dnd
 * 
 * ============================================================================ 
 */
...
import {
  ...
  ShoppingCartIcon, ReceiptIcon, ArrowRightIcon
...
const WORKFLOW_COLUMNS = [
  ...
  { id: 'in_progress', title: 'En cours', color: 'bg-purple-50 border-purple-200', icon: WrenchIcon },
  { id: 'waiting_parts', title: 'Attente pièces', color: 'bg-orange-50 border-orange-200', icon: ExclamationTriangleIcon },
  { id: 'ready', title: 'Prête', color: 'bg-green-50 border-green-200', icon: CheckCircleIcon },
...
const RepairsModule = () => {
  ...
  const [kanbanData, setKanbanData] = useState({
    pending: [], diagnosis: [], waiting_parts: [], in_progress: [], testing: [], 
  ...
  // Modification du statut avec boutons (alternative au drag & drop)
  ...
  const handleSendToCheckout = async (repair) => {
    try {
      // Créer une commande client à partir de la réparation
      const orderData = {
        client: repair.client?.id,
        store: repair.store,
        items: [
          {
            product: null, // Service de réparation
  ...
  // Rendu de la vue Kanban avec boutons (sans drag & drop)
              ...
              <div className="min-h-[200px] space-y-2">
                {columnRepairs.length === 0 ? (
                  ...
                  columnRepairs.map((repair) => (
                    <div
                      key={repair.id}
                      className="bg-white p-3 rounded-lg shadow-sm border cursor-pointer transition-all hover:shadow-md"
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="mt-3 pt-2 border-t border-gray-100">
                        <div className="flex gap-1">
                          {WORKFLOW_COLUMNS.filter(col => col.id !== column.id).map(nextColumn => (
                            <button
                              key={nextColumn.id}
                              onClick={(e) => {
                                e.stopPropagation();
                                handleStatusChange(repair.id, nextColumn.id);
                              }}
                              className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded transition-colors"
                              title={`Déplacer vers ${nextColumn.title}`}
                            >
